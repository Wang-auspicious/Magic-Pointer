...
def _draw_locator_stroke(
    ...
    *,
    ...
    tag_width = 16 + 8 * len(tag)
    draw.rounded_rectangle(
        (left, tag_top, left + tag_width, tag_top + 18), radius=5,
        fill=(30, 64, 175, 230),
    )
    draw.text((left + 7, tag_top + 3), tag, fill=(255, 255, 255, 255))
            ...
            if style == "locator":
                if polylines:
                    for index, line in enumerate(polylines):
                        _draw_locator_stroke(
                            draw,
                            [(x - bbox[0], y - bbox[1]) for x, y in line],
                            tag=chr(ord("A") + index),
                        )
                else:
                    _draw_locator_stroke(draw, local, tag="THIS")
