...
-   - Added `scripts/benchmark_voice_engines.py` (same recordings through both engines, CER via char-level Levenshtein, optional intent accuracy through the production RecipeRouter) plus contract/benchmark tests.- Started the intent-execution separation roadmap (advisor plan, Phase 1):
