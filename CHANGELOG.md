...
- Recut Stage motion against the measured `PromptRescue.mp4` transition: processing panels reveal from a 4.5% edge sliver, the completion shell settles first, body text sharpens from a 2.5px blur after 44ms, and `TASK FINISHED` lands last at 190ms. Settled cards now own follow-up/approval instead of leaving a duplicate input capsule visible. The disliked indeterminate strip is replaced by one neutral orbit dot; real determinate progress remains distinct.
- Replaced the broken `preferForeground` write shortcut with a single-process adaptive UIA resolver. `填入` now tries the focused editable control, the app under the cursor, the last stable external foreground app, the live foreground app, and finally the frozen source app; Magic Pointer windows, password fields, disabled controls, stale HWND/PID hints, unverified writes, and submit actions remain ineligible. The renderer still cannot choose a target.
- Restored real Explorer-file grounding in the active Stage path. File questions now freeze the selected absolute path, read actual text/PDF/DOCX/HTML/archive/directory content into the answer and Agent context (bounded at 60,000 characters with explicit truncation/error metadata), and send original image files to vision instead of guessing from filenames or analyzing an Explorer-row screenshot.
- Reworked transient stage surfaces from the Vida motion reference without copying its colorful generating strip: process/result panels now choose the free left or right app gutter with an 8 DIP gap, keep that side stable while streaming, and fall back to the screen edge opposite the focus region for fullscreen apps. Unknown progress is one neutral graphite activity dot and ink wash. Completion cards use 406/420/560/840 DIP content tiers and integrate write-back approval into the `TASK FINISHED` footer. Reference collages no longer alter or truncate real folder/file proposal data.
...
- Fixed the Windows gesture cursor without adding software-follow latency: replaced the renderer-driven DOM cursor with a native `armed-cursor.cur` resource (SVG fallback on other platforms). The OS now moves the cursor directly; real desktop verification requires one stable, nonzero Windows cursor handle from pointer-down through release while also checking visible drawing and capsule delivery.
...
- Migrated the automatic-update manager to strict TypeScript, covering exact channel selection, semver acceptance, concurrent checks, download prompts, progress, and timer disposal.
- Removed the unreferenced legacy panel-position algorithm and its isolated test; live stage placement remains covered by the active anchor, hit-region, and display tests.
- Migrated pass-through gesture capture to strict TypeScript, preserving click-through edges, right-click dismissal, release samples, and multi-stroke completion.
