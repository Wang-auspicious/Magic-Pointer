...
stem=ROOT/"artifacts/selection-sovereign-replay-20260918-fullcontext"
...
report={
    "validation":"current production Look with selected detail plus same-frame full visual context; fresh OCR and one production _loop_router replay",
...
with stem.with_suffix(".progress.log").open("w",encoding="utf-8") as progress:
    clock=bridge.PhaseClock("selection-fullcontext-acceptance",stream=progress)
    try:
        started=time.perf_counter()
        engine=RapidOCR()
