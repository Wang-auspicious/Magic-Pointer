/**
 * The four genuine SVG marks in Claude's UI; the rest of its iconography is a
 * font and is handled elsewhere.
 *
 * Source: 参考claude设计/scraped/icons/
 *   spark.svg, voice-activity.svg, incognito.svg, reflect-broadcast.svg
 *
 * Each string below is the scraped file's own bytes: path data, attribute
 * order, decimals and stroke-width as they were found. The one mechanical edit
 * rewrites the fixed width/height pair on the root <svg> to 100%, so a mark
 * fills whatever box it lands in. Nothing here is redrawn or geometrically
 * approximated.
 *
 * All four scraped files carry a viewBox, so none of them needed one computed
 * from its width/height. The newline each mark ends with is its file's own
 * final byte.
 */

type MarkName = 'spark' | 'voiceActivity' | 'incognito' | 'reflectBroadcast';
...
`,
  voiceActivity: `<svg width="100%" height="100%" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" xmlns="http://www.w3.org/2000/svg" class="block overflow-visible" aria-hidden="true"><g><line x1="2.5" y1="8.5" x2="2.5" y2="11.5" class="idle-activity-bar"></line><line x1="5.5" y1="6.5" x2="5.5" y2="13.5" class="idle-activity-bar"></line><line x1="8.5" y1="3.5" x2="8.5" y2="16.5" class="idle-activity-bar"></line><line x1="11.5" y1="6.5" x2="11.5" y2="13.5" class="idle-activity-bar"></line><line x1="14.5" y1="4.5" x2="14.5" y2="15.5" class="idle-activity-bar"></line><line x1="17.5" y1="8.5" x2="17.5" y2="11.5" class="idle-activity-bar"></line></g></svg>
`,
...
// Keys stay camelCase because they are JS identifiers; each mark is named
// after the file it was scraped from.
...
function svg(name: string, options: MarkOptions = {}): string {
  ...
  // Some files bring their own class attribute — and a nested <g> brings
  // another — so the merge is confined to the root tag.
  const tag =
    close === -1
      ? `${head} class="${merged}"`
