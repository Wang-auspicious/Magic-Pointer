...
const ChatTrajectory = (() => {
  ...
  function timeline(rows: TrajectoryRow[]): TrajectoryNode {
    const root = h('section', { class: 'mp-chat-trajectory-timeline', 'aria-label': 'Trajectory timeline' });
    const plot = h('div', { class: 'mp-chat-trajectory-plot' });
    append(plot, h('div', { class: 'mp-chat-trajectory-lane-labels', 'aria-hidden': 'true' },
      h('span', {}, 'Input'), h('span', {}, 'Model'), h('span', {}, 'Tools')));
    const track = h('div', { class: 'mp-chat-trajectory-track', tabindex: '0' });
    const lanes = h('div', { class: 'mp-chat-trajectory-lanes' });
    rows.forEach((row, index) => {
      append(lanes, h('span', {
        class: 'mp-chat-trajectory-span', 'data-timeline-span': row.kind,
  ...
  function toolbar(): TrajectoryNode {
    const root = h('div', { class: 'mp-chat-trajectory-toolbar', role: 'toolbar', 'aria-label': '轨迹工具栏' });
    const inner = h('div', { class: 'mp-chat-trajectory-toolbar-inner' });
    const actions = h('div', { class: 'mp-chat-trajectory-actions' });
    const durationButton = h('button', {
      type: 'button', class: 'mp-chat-trajectory-toggle', 'data-trajectory-action': 'duration',
      'aria-pressed': 'true', title: 'Use equal-width operations',
    });
    append(durationButton, s('svg', { class: 'mp-chat-trajectory-duration-icon', viewBox: '0 0 16 16', fill: 'none', 'aria-hidden': 'true' },
    ...
    append(actions, h('button', {
      type: 'button', class: 'mp-chat-trajectory-action', 'data-trajectory-action': 'turns',
      'aria-pressed': 'false', title: 'Collapse turns',
    }, h('span', { class: 'mp-chat-trajectory-action-icon', 'aria-hidden': 'true' }, '⊟'), 'Turns'));
    append(actions, h('button', {
      type: 'button', class: 'mp-chat-trajectory-action', 'data-trajectory-action': 'calls',
      'aria-pressed': 'false', title: 'Collapse calls',
    }, h('span', { class: 'mp-chat-trajectory-action-icon', 'aria-hidden': 'true' }, '⊟'), 'Calls'));
    append(inner, actions);
    const search = h('label', { class: 'mp-chat-trajectory-search' });
    if (typeof ChatIcons !== 'undefined') append(search, ChatIcons.node('search', 11));
    append(search, h('input', {
      type: 'search', class: 'mp-chat-trajectory-search-input', 'data-trajectory-search': '',
  ...
  function eventIcon(kind: TrajectoryKind): TrajectoryNode {
    ...
    return h('span', { class: 'mp-chat-trajectory-kind-dot', 'aria-hidden': 'true' });
  }

  function ledger(rows: TrajectoryRow[]): TrajectoryNode {
    const split = h('div', { class: 'mp-chat-trajectory-split' });
    const pane = h('div', { class: 'mp-chat-trajectory-table-pane', 'data-trajectory-scroll': '' });
    const table = h('table', { class: 'mp-chat-trajectory-table', 'aria-rowcount': String(rows.length) });
    append(table, h('colgroup', {}, h('col', { class: 'mp-chat-trajectory-event-column' }), h('col', { class: 'mp-chat-trajectory-content-column' })));
    const body = h('tbody');
    let lastTurn = 0;
    rows.forEach((row, position) => {
      ...
      const event = h('td', { class: 'mp-chat-trajectory-event' });
      if (turnStart) {
        append(event, h('span', { class: 'mp-chat-trajectory-turn-label', 'aria-label': `Turn ${row.turn}` }, `Turn ${row.turn}`));
        append(event, h('span', { class: 'mp-chat-trajectory-turn-rail', 'aria-hidden': 'true' }));
        lastTurn = row.turn;
      }
      const eventInner = h('div', { class: 'mp-chat-trajectory-event-inner' });
      const tag = h('span', { class: `mp-chat-trajectory-kind-tag mp-chat-trajectory-kind-${row.kind}`, 'data-role-kind': row.kind });
      append(tag, h('span', { class: 'mp-chat-trajectory-kind-icon', 'aria-hidden': 'true' }, eventIcon(row.kind)));
      append(tag, h('span', { class: 'mp-chat-trajectory-kind-label' }, KIND_LABEL[row.kind]));
      append(eventInner, tag);
      append(event, eventInner);
      append(rowNode, event);
      const content = h('td', { class: 'mp-chat-trajectory-content' });
      const text = h('span', { class: row.result === undefined ? 'mp-chat-trajectory-content-text' : 'mp-chat-trajectory-result-preview' });
      if (row.kind === 'tool') {
        const request = row.result === undefined
          ? text
          : h('span', { class: 'mp-chat-trajectory-result-request' });
        append(request, h('span', { class: 'mp-chat-trajectory-tool-name' }, row.label));
        if (row.text) append(request, h('span', { class: 'mp-chat-trajectory-tool-payload' }, row.text));
        if (request !== text) append(text, request);
      } else append(text, row.text || '—');
      if (row.result !== undefined) append(text, h('span', { class: row.failed ? 'mp-chat-trajectory-inline-result is-error' : 'mp-chat-trajectory-inline-result' },
        h('span', { class: 'mp-chat-trajectory-arrow' }, '→'),
        h('span', { class: 'mp-chat-trajectory-inline-result-text' }, row.result || 'No output')));
      append(content, text);
  ...
  function bind(root: Element): void {
    ...
    for (const name of ['calls', 'turns']) {
      ...
      button?.addEventListener('click', () => {
        ...
        const icon = button.querySelector('.mp-chat-trajectory-action-icon');
    ...
    search?.addEventListener('input', () => {
      ...
      root.querySelectorAll<HTMLElement>('.mp-chat-trajectory-table tbody tr').forEach(row => {
  ...
  function render(rows: TrajectoryRow[]): TrajectoryNode {
    const root = h('div', { class: 'mp-chat-trajectory', 'data-actual-duration': 'true' });
    append(root, toolbar());
    append(root, timeline(rows));
    append(root, h('div', { class: 'mp-chat-trajectory-ledger' }, ledger(rows)));
...
if (typeof module !== 'undefined' && module.exports) module.exports = ChatTrajectory;
