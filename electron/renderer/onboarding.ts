...
const stages = new Map<string, MagicPointerOnboardingStage>();
...
function renderStages() {
  ...
  const rows = stageOrder.map((id) => {
    ...
    title.textContent = (stage.title || stage.id) as string;
    const state = document.createElement('span');
    state.className = 'onboarding-stage__state';
    state.textContent = stateLabels[stage.state as string] || '';
