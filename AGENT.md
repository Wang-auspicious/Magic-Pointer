...
- **写回确认属于 `TASK FINISHED` 卡片。** 待写文本仍同步回胶囊输入框供编辑，但「写回哪个应用 / 拒绝 / 同意」长在完成卡底栏；不要再创建跟胶囊分离的第二个审批浮层。完成卡按内容使用 context/compact/normal/wide 四档。外部参考图里的九宫格只用于学习质感与组件语言，绝不能据此新增、截断或改造产品文件预览。
