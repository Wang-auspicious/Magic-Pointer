...
### 2026-08-04 晚 — 「填入」落地：气泡里的答案能搬进别的应用了；顺带实测出微信是零元件

**用户的问题**：气泡里既然已经有结果，能不能直接把它搬进微信聊天框？不行就先放剪贴板再粘。**答案：能，而且底层早就写好了**——`_paste_text_to_foreground`（`app/actions/executor.py:311`）是一条带完整身份校验的跨应用写入通道，此前只服务于「把 prompt 填进 Agent 输入框」。这一轮把它接到了气泡上。

**新增（Python 741 passed / Node 119 tests）**：

- 气泡工具条多了一个**「填入」**按钮（`electron/renderer/stage.html` 的 `thread-insert`）。它把**屏幕上那份文字**（`resultPlainText`，所以用户改过/追问过的版本才是搬走的那份）交给主进程；**目标窗口、pid、标题、坐标一律由主进程从冻结的 selection session 取**，渲染进程无法指定写到哪里。这条边界有静态测试钉住（`tests/stage_insert_result_text_test.js`）。
- `app/actions/capsule_delivery.py` + `scripts/deliver_text_bridge.py`：写入成功且**读回校验通过**才说「已填入并核对过」；否则一律降级到剪贴板，并**说出具体原因**（不是输入框 / 框里已有别的内容 / 密码框 / 终端 / 读不回来无法确认 / 没有可信目标）。`write_attempted` 区分「键已经按下去了但确认不了」和「根本没动手」——前者必须告诉用户「已经粘了一次，自己看一眼」，后者不能这么说。
- 保留了两条 writer 的既有拒绝，它们是特性不是缺陷：**不覆盖用户自己打的草稿**、**密码框永不写入**。撞上就走剪贴板。

**真机实测出来的硬事实（用户问「微信的对话框能不能像 Everywhere 那样框出来」）**：

| 窗口 | 整窗 UIA 元件扫描结果 |
|---|---|
| Token Monitor（普通窗口） | **64 个元件**，各带精确方框（`ControlType.Button [2446,108,622,80]` 这种） |
| 微信 4.0（`Weixin.exe`, class `WeChatMainWndForPC`） | **0 个元件**，整窗、下半区、输入区都是 0 |

所以：**框高亮我们已经有了**（`stage.css` 三种样式，`sweep_band` 就是光带绕框跑一圈，rect 来自 `artifacts.selection_rectangles`），浏览器里的推特帖子这类能框；**微信框不出来，不是我们没做，是它一个元件都不向系统汇报**。微信只能走「看图」路线（截图 + 元件检测模型，`catalog.py:62` 已把 `omniparser` 列为 provider），那是另一条链路的活。

**连带确认**：这台机器是 **200% 缩放**（`DESKTOPHORZRES=3120 / HORZRES=1560`）。用 PowerShell 的 `GetWindowRect` 拿到的是**逻辑像素**，探针是 DPI-aware 要**物理像素**，差 2 倍。我第一次就是这么把点打到了另一个窗口上——探针的身份校验正确地拒绝了它（`error` 里带 `chain=` 显示实际命中哪个窗口，调试时很有用）。
...
**先看清现在的位置**：「填入」（2026-08-04 晚）已经把跨应用写入这条路走通了，但它是**投递**（把气泡答案写进一个输入框），不是**原位替换**（把用户选中的那段字换掉）。两者的写入机制不同：投递可以要求目标为空，替换必须精确覆盖活动选区。所以下面这件事仍然要做，只是不再是从零开始 —— `app/actions/capsule_delivery.py` 里的「三态诚实口径」和错误→原因映射可以直接复用，级别 3 的措辞已经写好了。

**微信的级别 3 现在是实测确认，不是假设**：整窗 UIA 元件扫描返回 **0 个元件**（同一工具在普通窗口返回 64 个）。也就是说微信不存在「读回校验」的可能性，`written_unverified` 不是保守设计而是唯一诚实的选项。
...
6. **「填入」逐应用验（2026-08-04 晚新增）**：在记事本 / Chrome 输入框 / 微信输入框里各走一次「划线 → 提问 → 点填入」。要看四件事：① 记事本这类应当显示「已填入输入框并核对过内容」；② 微信应当显示「已经粘贴了一次…无法确认…请自己看一眼」**而不是**「已填入」；③ 剪贴板里原有的图片/文件没被毁；④ 任何情况下都没有替你按发送。日志找 `stage:insert-result-text outcome=`，`reasonCode` 就是判定原因。
...
- **不要试图用 UIA 去框微信的界面元素**：实测整窗 0 个元件（同一工具在普通窗口 64 个）。微信只能走截图+元件检测模型，走接口是白费。
- **不要用 PowerShell 的 `GetWindowRect` 结果直接喂探针**：这台机器 200% 缩放，PS 拿到逻辑像素，探针要物理像素，差 2 倍会打到别的窗口上去。
...
## 已记下、还没做的（TODO，别当成已完成）

按纪律 1（不死磕），这些是做到"可用"后主动记下来换任务的，不是忘了：

| 事项 | 现状与下一步 |
|---|---|
| **token 用量热力图** | 计划里写"数据 fabric-audit.jsonl 里已经在记"——**实测是错的**：376 条事件里零个 token 字段。使用情况页现在按能力统计次数（真实数据）。要做热力图得先让 `ask_text_model` / `ask_text_model_with_tools` 把 usage 写进审计日志。 |
| **点选改成 hover 触发** | `element_probe_bridge.py` 每次点选 spawn 一个 Python（~1.2s，其中 ~300ms 是解释器启动）。点击够用，hover 不够。参照 `ocr_resident_worker.py` 做常驻即可解锁。 |
| **气泡在第一笔就打开** | 流式 chip 的记账（`stage_turn_stream.js`）已经完成并测过，但气泡仍然是画完才开，所以 chip 是"打开时一次给全"而不是"每笔 250ms 冒一个"。改的是手势生命周期，不是 AI。 |
| **感知级联并行化** | 手势兜底采样已加 3.5s 预算（12.9s → ~4s）。剩下的空间在于并行跑适配器而不是串行，以及 UIA 探针常驻化。当前量级可用，先不磕。 |

---

## 工作方式：用 mattpocock 的工程技能，别靠记忆蛮干

用户明确要求：**推进过程中反复使用这几个技能**，不是开工前看一眼就忘。它们装在
`~/.claude/skills/`，`/ask-matt` 是它们的路由器（忘了该用哪个就问它）。

本项目最常用的四个，按触发时机：

| 时机 | 技能 | 为什么是它 |
|---|---|---|
| 要写一个新行为（每个新能力、每条 recipe） | **`/tdd`** | 一次一个红绿切片。本项目最贵的一次教训就是"自动测试全绿 + 真机全挂"，先写会失败的测试是唯一防线 |
| 提交前，diff 已经成型 | **`/code-review`** | 双轴审查（Standards + Spec）。红线第 7 条"每功能一提交"配这个才有意义 |
| 遇到查不动的 bug（间歇、回归、看不出根因） | **`/diagnosing-bugs`** | 它拒绝在拿到"一条能稳定复现的红色命令"之前猜原因——2026-08-04 那次四场景全挂，正是因为先猜后验白绕了一圈 |
| 要决定一个模块该长什么样（新增 provider / 拆 selection_bridge） | **`/codebase-design`** | deep module 词汇：小接口、干净 seam。`selection_bridge.py` 已经 2000 行，任何新增都该先过这一关 |

补充规矩：

- **别跳过 `/tdd` 直接写实现**，哪怕"这个很简单"。P0 修复里 `image.to_prompt`
  少了 provider 映射就是没先写测试，跑到第 166 个测试才炸。
- **`/code-review` 在 commit 之前跑，不是之后**。提交完再审只能产生第二个提交。
- 术语打架时（"对象/选区/THIS/element" 混用）用 **`/domain-modeling`** 收敛，
  别在代码里各写各的。
- 这些技能的产物（ADR、CONTEXT.md）如果本仓库还没建，不要为了用技能而硬造目录结构；
  先按技能的思路做事，落盘位置沿用 `docs/planning/`。
