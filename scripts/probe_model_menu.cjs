'use strict';
// Chromium input/layout regression. Fixture only supplies the model catalog.
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    check(await wc.executeJavaScript("document.querySelectorAll('[data-model-id] .dshw-model-source').length===4"), 'each main model needs its provider badge');
    ...
    check(await wc.executeJavaScript("document.querySelectorAll('[data-model-pin] .dshw-model-source').length===29"), 'each More model needs its provider badge');
    ...
    await wc.executeJavaScript(`document.querySelector('.dshw-model-more-panel').scrollTop=10000`);
    ...
    await wc.executeJavaScript("document.querySelector('.dshw-model-more-panel').scrollTop=0");
