...
const referenceConversations = [
  {
    ...
    object: { app: 'Studio', windowTitle: 'Greeting', label: 'VisLexicon 视元' },
    turns: [
      {
        at: NOW - 12 * 60 * 1000,
        question: '你好',
...
const projectEntries = {
  ...
  'electron/renderer': [
    { name: 'chat_styles.css', path: 'electron/renderer/chat_styles.css', kind: 'file' },
    { name: 'studio_layout.css', path: 'electron/renderer/studio_layout.css', kind: 'file' },
    { name: 'theme_tokens.css', path: 'electron/renderer/theme_tokens.css', kind: 'file' },
...
contextBridge.exposeInMainWorld('magicPointerDashboard', {
  ...
  projects: {
    ...
    environment: async (root) => String(root).replace(/\\/g, '/').includes('Magic Pointer') ? ({
      ...
      fileChanges: [
        { path: 'electron/renderer/studio_layout.css', status: 'M', staged: false },
  ...
  conversations: {
    ...
    artifacts: async () => [{ name: 'Studio', kind: 'design', at: NOW, conversationId: 'studio-reference' }],
