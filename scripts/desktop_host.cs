...
internal static class DesktopHost
{
    ...
    static void Type(string text){int index=0;foreach(char c in text){if(index++%8==0)CheckUserInterference();Send(new Input{type=1,data=new InputUnion{keyboard=new Keyboard{scan=c,flags=4}}},new Input{type=1,data=new InputUnion{keyboard=new Keyboard{scan=c,flags=6}}});}}
    ...
    static object Act(IDictionary<string,object> a){string action=Str(a,"action");long hwnd=Convert.ToInt64(Get(Obj(Get(a,"window")),"hwnd",0));
        ...
        using(var mutex=new Mutex(false,"Local\\MagicPointer.RealInput")){bool owned=false;try{try{owned=mutex.WaitOne(0);}catch(AbandonedMutexException){owned=true;}if(!owned)throw new Exception("computer_use_busy");
            ...
            else if(action=="drag"){int tx=Num(a,"to_x"),ty=Num(a,"to_y");CheckTarget(a,false,tx,ty);int duration=Math.Max(0,Math.Min(3000,Num(a,"duration_ms",200)));var points=Arr(Get(a,"path")).Select(Obj).ToArray();foreach(var point in points)CheckTarget(a,false,Num(point,"x"),Num(point,"y"));try{MouseInput(2);if(points.Length>1){foreach(var point in points){CheckUserInterference();MoveCursor(Num(point,"x"),Num(point,"y"));Thread.Sleep(duration/points.Length);}}else for(int i=1;i<=20;i++){CheckUserInterference();MoveCursor(x+(tx-x)*i/20,y+(ty-y)*i/20);Thread.Sleep(duration/20);}}finally{MouseInput(4);}}
