...
**一句话定义**（设计文档 §1.2，2026-08-19 用户纠正后）：
...
- **不是**：更便宜的 CUA、7×24 录屏、整屏上传。
- **任务时长不是边界**：短任务与长程任务都由 MP 自有 Runtime 承担。本节原写的"内部任务=一两轮、几分钟"和"项目级交给 Claude Code/Codex/Pi"已于 2026-08-19 被用户推翻。为外部客户端编译 prompt 填入输入框，只是一条与微信输入框同级的投递通道。
