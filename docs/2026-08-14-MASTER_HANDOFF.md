...
### 1.1 一句话定义（2026-08-19 用户纠正后）

> **Magic Pointer 是一个完整的桌面 Agent Harness：把人的桌面指代理解预编译成 Agent 可直接执行的上下文，再用自有 Runtime 承担从一轮到长程的全部任务。**

原句写的是“预编译为**短任务** Agent 可直接执行上下文”，已于 2026-08-19 被用户推翻：任务时长不是产品边界。
...
- 产品重心不在仓库级代码重构的产品化体验；但这是重心，**不是能力上限**，不得据此给 Runtime 加轮次/时长/复杂度封顶。
- 为 Claude Code / Codex 编译 prompt 填入输入框，是一条投递通道（与写进微信输入框同级），不是任务分级，也不移交执行权。
- 任务 = 短任务与长程任务都算：圈选生成回复、OCR/改写/翻译/表格、打开应用/调音量，以及批量处理、盯守长过程、跨小时推进并可续跑的作业。
