...
**开发树 1.0.11；本机安装版仍是 1.0.10（本轮 sync 未跑通，尚未交付）。** Magic Pointer 的路线是完整自有 Agent：Studio 保留已交付的 DSH 高保真工作面，自有 Runtime 承担短任务，确定性感知/权限/执行边界归 MP；Hermes/Pi 只作为持续对照和资产语义来源。感知输入已有并发结构化证据融合（带裁决 deadline）与 InputArtifact；运行内核已有 effect sandwich、durable `next-step`/`next-turn` Inbox、本地 session bridge，以及从唯一 EventSession 投影的 interaction ledger，selection/Studio 均返回公开账单；崩溃后的恢复指引按每个操作自己的 effect 分级发给模型。开发树验证：**Python 1318 过；Node 151 全过（104 个源文件）；五套 typecheck 与 ESLint 通过**；安装版交付（NSIS 构建 + `npm run sync` + 安装目录版本核对）本轮**未完成**，仍是 1.0.10 那次的结果。OCR/Vision/SurfaceAdapter 的统一 Broker、运行中插话 GUI、ledger 可视化、可续跑 crash resume、DraftArtifact revision 与 ask-user UI 往返仍是明确后续，不能把两批地基冒充成完整 Agent 已经做完。

蓝图 §13.1 把 `app/perception/providers.py`（provider 协议与 adapter bridge）和 `app/perception/fusion.py` 标为第一批实现，实际二者都不存在：融合逻辑内嵌在 `broker.py`，provider 边界仍是 `adapter.read_context()` 的隐式 duck typing。这正是 OCR/Vision/Explorer 进不了同一次 fan-out 的原因。按蓝图 §7.2 自己的告诫（"不要一开始做复杂泛型框架"）与"两个实现才让接缝成真"，抽这层协议应当与接入第二类 provider 同批做，而不是先立一个只有一个实现的抽象。
