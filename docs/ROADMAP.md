...
- **视觉基线：结束 `stage.css` 的石墨黑 / 浅蓝白分叉。** 规范（`docs/archive/superpowers/specs/2026-07-26-*`）写的是石墨黑 #0E1116，**实际发布的气泡是浅色**——改配色前先决定以哪个为准，不要只改一半。方向：气泡与浮层用浅色玻璃，设置中心用深色 mica，一套 accent 色贯穿可在外观页改。图标用 lucide（MIT），别从表情库随便捡。
