...
## 代码库收口 / TypeScript 迁移（2026-08-09 进行中）
...
- **~~按 PromptRescue 逐帧重做过程→完成转场~~（2026-08-09 已完成）**：左右自适应停靠不变；过程面板从边缘细缝展开，完成外壳/正文/状态按 0ms/44ms/190ms 分层落下，正文由糊到清，`TASK FINISHED` 最后出现。结果态移除重复胶囊，未知进度条换成中性单点轨道，reduced-motion 与静态视觉契约已覆盖。
- **~~修复「填入」目标解析~~（2026-08-09 已完成）**：删除无条件 `preferForeground` 捷径；同一个原生 writer 内按“聚焦输入框 → 鼠标所在应用 → 稳定前台 → 实时前台 → 原始应用”解析，找不到现成焦点时在候选窗口内选最佳可编辑控件。主进程提示不经过 renderer，实际目标变化只有带原生解析证据的 adaptive receipt 才能通过执行器；密码/禁用/本产品窗口/no-submit/读回校验边界不变。
...
- **Vida 临时界面动效与自适应停靠（2026-08-09 重做中）**：保留按目标应用外侧空位贴左/贴右、全屏避开选区和卡内审批底栏；撤销误加的 3×3 文件预览。下一步按 PromptRescue 的真实帧序重做生成胶囊到完成卡的连续变形，并用参考拼图只校准材质、间距与组件语言。
