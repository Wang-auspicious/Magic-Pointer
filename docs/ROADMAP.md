...
- **~~Vida 临时界面动效与自适应停靠~~（2026-08-09 已完成）**：过程/结果面板按目标应用外侧空位贴左或贴右，全屏时贴屏幕边缘并避开选区；同一会话保持侧边稳定，手动拖动优先。彩色未知进度条替换为中性石墨点轨道；`TASK FINISHED` 使用内容宽度四档、卡内审批底栏和最多九项的 3×3 预览。剩余视觉工作属于设置中心/外观页，不再回头改这套舞台基线。
