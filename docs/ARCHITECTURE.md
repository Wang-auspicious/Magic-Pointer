...
**过程与完成共用一块自适应边缘表面，结果态不保留第二个胶囊。** 面板位置由 `chooseAdaptivePanelAnchor` 按源应用左右空位决定，同一会话固定 side；全屏时才落到焦点反侧的屏幕边缘。出现时从靠应用的边缘细缝展开。完成阶段以参考视频 60fps 帧序为准：外壳/动作先落，正文 44ms 后由糊到清，状态眉毛 190ms 后最后出现。处理胶囊仅在运行中保留，负责展示命令和停止动作；完成后卡内追问/审批成为唯一交互面。所有阶段在 reduced-motion 下取消动画，且不依赖 GSAP、定时假进度或彩色未知进度条。
