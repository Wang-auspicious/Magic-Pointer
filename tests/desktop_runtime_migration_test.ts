...
const { validateFrameLease } = require('../electron/frame_lease');
const { textsAgree, selectOpenStroke, registerPerceptionTools, registerLookTool } = require('../electron/runtime/desktop_perception');
...
async function main() {
  const registry = new ToolRegistry(); registerDesktopTools(registry); registerPerceptionTools(registry, { snapshot: {} }); registerLookTool(registry, { snapshot: {} }); assert.ok(registry.get('choose_ui_target')); assert.ok(registry.get('look')); assert.equal(registry.get('type_text').effect_for({ submit: true }), 'external_send');
  ...
  const order: string[] = [];
  const service = new FrameCaptureService(root, 1, async () => { order.push('capture'); count++; if (count === 1) return { bytes: Buffer.from('historical'), width: 10, height: 10, source: 'test', capturedAtUtc: new Date().toISOString() }; return new Promise(resolve => { complete = resolve; }); }, async () => { order.push('window'); return { hwnd: 42, pid: 7, processId: 7, processName: 'demo.exe', processStartTime: '2026-09-23T00:00:00.000Z', title: 'Demo', bbox: [1, 1, 9, 9] }; });
  try {
    service.arm({ epochId: 'one', displayId: 'one', surfaceBoundsPx: [0, 0, 10, 10], targetWindow: { hwnd: 42, processId: 7, processName: 'demo.exe', title: '' } });
    ...
    assert.deepEqual(order.slice(0, 2), ['capture', 'window']);
    assert.deepEqual(validateFrameLease(frozen).targetWindow.bbox, [1, 1, 9, 9]);
    assert.equal(validateFrameLease(frozen).targetWindow.processStartTime, '2026-09-23T00:00:00.000Z');
