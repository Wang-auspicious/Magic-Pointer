...
const DshHighlight = require('../electron/renderer/dsh_highlight');
...
// Every case goes through here first. A line's spans, concatenated, must
// reproduce that line character for character — that is the invariant the
// renderer depends on — and every token must be one of the eight categories.
const check = (code, lang) => {
  const result = DshHighlight.highlight(code, lang);
...
// [text, token] for every whitespace-separated piece of the first line. Merging
// only ever joins spans that already share a token, so splitting a span's text
// on whitespace keeps each piece with the right label.
const words = (code, lang) => {
  const out = [];
  for (const span of DshHighlight.highlight(code, lang)[0]) {
...
const tokenFor = (code, lang, text) => {
  const found = DshHighlight.highlight(code, lang)[0].find((span) => span.text === text);
...
// CRLF: the \r is stripped from the end of every line, and nothing else moves.
...
// A carriage return that is not a line ending is still emitted.
assert.deepStrictEqual(
  DshHighlight.highlight('a\rb', 'shell')[0].map((span) => span.text).join(''), 'a\rb');
...
// Neighbouring spans that share a token are merged, so `];` arrives as one
// operator span rather than two.
...
// ------------------------------------------------- role decides, not the word

// `export` and `local` are in the shell keyword list and are also commands: in
// command position the role wins, which is the rule the spec calls out.
...
// The same word is a keyword in keyword position and an argument elsewhere.
assert.strictEqual(tokenFor('x; fi', 'shell', 'fi'), 'keyword');
assert.deepStrictEqual(words('echo fi', 'shell'), [['echo', 'command'], ['fi', 'plain']]);
// PowerShell: a word at the start of a statement is a command unless it is a
// keyword, and a word before a -Flag is a command wherever it sits.
...
// The `-Flag` lookahead is applied to every word, as specified: whatever sits
// immediately before a parameter is taken to be the command that owns it.
...
// Unterminated: the rest of the line stays one string span, and it round-trips.
assert.deepStrictEqual(
  DshHighlight.highlight('Write-Host "never closed', 'powershell')[0].slice(2),
  [{ text: '"never closed', token: 'string' }]);
// A quoted space stays inside the string span, so this case is asserted on the
// spans themselves rather than through the whitespace-splitting helper.
assert.deepStrictEqual(DshHighlight.highlight('echo "a b" c', 'shell'),
...
// Flags are commands in both languages.
...
// A bare `$` is not a variable and must still round-trip.
assert.deepStrictEqual(
  DshHighlight.highlight('echo $', 'shell')[0].map((span) => span.text).join(''), 'echo $');

// ----------------------------------------------------------------- comments

assert.deepStrictEqual(DshHighlight.highlight('# a note', 'powershell'),
  [[{ text: '# a note', token: 'comment' }]]);
assert.deepStrictEqual(DshHighlight.highlight('echo hi # a note', 'shell'),
  [[{ text: 'echo', token: 'command' }, { text: ' hi ', token: 'plain' },
    { text: '# a note', token: 'comment' }]]);
// In shell a `#` only opens a comment at the start of a line or after a space.
...
// A dash that starts a flag is not an operator.
assert.deepStrictEqual(words('-lt x', 'powershell'), [['-lt', 'command'], ['x', 'plain']]);

// ------------------------------------------------------------------ non ASCII

assert.deepStrictEqual(DshHighlight.highlight('echo 中文 # 中文注释', 'shell'),
  [[{ text: 'echo', token: 'command' }, { text: ' 中文 ', token: 'plain' },
    { text: '# 中文注释', token: 'comment' }]]);
assert.deepStrictEqual(
  DshHighlight.highlight('# 中文注释', 'powershell'),
  [[{ text: '# 中文注释', token: 'comment' }]]);
...
assert.deepStrictEqual(DshHighlight.highlight('a\nb', 'plain'),
  [[{ text: 'a', token: 'plain' }], [{ text: 'b', token: 'plain' }]]);

// ---------------------------------------------------------------- never throw

assert.doesNotThrow(() => DshHighlight.highlight(undefined, 'shell'));
assert.doesNotThrow(() => DshHighlight.highlight(null, 'powershell'));
assert.deepStrictEqual(DshHighlight.highlight(undefined, 'shell'),
  [[{ text: '', token: 'plain' }]]);
assert.deepStrictEqual(DshHighlight.highlight(42, 'shell'), [[{ text: '42', token: 'number' }]]);
assert.doesNotThrow(() => DshHighlight.langFor(undefined, undefined));
assert.strictEqual(DshHighlight.langFor(undefined, undefined), 'plain');
...
const LANG_CASES = [
  ...
  // The command text is checked before a shell tool name, so a PowerShell
  // command that arrives through the generic shell tool still tokenizes.
  [['bash', 'Write-Host hi'], 'powershell'],
];
for (const [[toolName, command], expected] of LANG_CASES) {
  assert.strictEqual(DshHighlight.langFor(toolName, command), expected,
    `${toolName} / ${command}`);
}
// A lowercase `get-` is not the `Get-` marker.
assert.strictEqual(DshHighlight.langFor('run_command', 'git log --no-merges target-x'), 'plain');
...
// Worst case for the span merge: every character is its own token.
...
// Every published token category was produced by the cases above.
assert.deepStrictEqual([...SEEN].sort(), [...TOKENS].sort());

console.log('dsh highlight test ok');
