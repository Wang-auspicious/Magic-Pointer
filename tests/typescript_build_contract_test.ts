...
const rendererConfig = JSON.parse(read('tsconfig.renderer.json'));
const toolsConfig = JSON.parse(read('tsconfig.tools.json'));
const scriptsBuildConfig = JSON.parse(read('tsconfig.scripts-build.json'));
...
assert.strictEqual(rendererConfig.extends, './tsconfig.json');
assert.strictEqual(toolsConfig.extends, './tsconfig.json');
assert.strictEqual(scriptsBuildConfig.extends, './tsconfig.json');
assert.strictEqual(testsConfig.extends, './tsconfig.json');
assert.deepStrictEqual(electronConfig.include, ['electron/**/*.ts']);
assert.deepStrictEqual(rendererConfig.include, ['electron/renderer/**/*.ts', 'electron/renderer/**/*.d.ts']);
