...
assert(html.includes('aria-label="关闭"'));
...
assert(source.includes('function renderCaptureEligibility('));
assert(source.includes('captureEligibility.commandReady === false'));
assert(source.includes('primaryIntentButton.disabled = true'));
assert(source.includes('commandRow.hidden = true'));
assert(source.includes('captureEligibility.autoDismissMs'));
assert(source.includes("closeButton.addEventListener('click'"));
