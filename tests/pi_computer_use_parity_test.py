"""Pi computer-use parity contracts for Magic Pointer's native desktop seam.

These are intentionally provider-neutral tests: no live desktop is touched.
They pin the state-scoped root/element tools, transactional act, and bounded
condition waiting that ``pi-computer-use`` exposes to its model.
"""
