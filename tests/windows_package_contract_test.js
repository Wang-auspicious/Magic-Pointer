...
for (let index = 0; index < imageCount; index += 1) {
  ...
  assert.strictEqual(planes, 1, `${width}x${height} ICO frame must declare one Windows color plane`);
  icoSizes.add(`${width}x${height}`);
}
for (const size of [16, 24, 32, 48, 64, 128, 256]) {
  assert(icoSizes.has(`${size}x${size}`), `ICO must contain ${size}x${size} frame`);
}
assert(/win:\s*[\s\S]*?icon:\s*assets\/app\/icon\.ico/.test(builder),
  'win.icon must point at the shipped Magic Pointer ICO');
assert(/files:\s*[\s\S]*?-\s*assets\/app\/icon\.ico/.test(builder),
  'the packaged tray runtime must contain the Magic Pointer ICO');
assert(!builder.includes('- scripts/**'),
...
for (const runtimeScript of [
  ...
  assert(builder.includes(`- ${runtimeScript}`), `package must include runtime script ${runtimeScript}`);
}
assert(!builder.includes('scripts/capture_dashboard.js'),
  'package must not contain the development capture script with local workspace provenance');
assert(/extraResources:\s*[\s\S]*?from:\s*build\/python-runtime[\s\S]*?to:\s*python-runtime/.test(builder),
  'packaged app must ship build/python-runtime as resources/python-runtime');
assert(/installerIcon:\s*assets\/app\/icon\.ico/.test(builder),
  'NSIS installer must use the app ICO');
assert(/uninstallerIcon:\s*assets\/app\/icon\.ico/.test(builder),
  'NSIS uninstaller must use the app ICO');
assert(/createDesktopShortcut:\s*true/.test(builder),
  'the end-user installer must create a double-click desktop shortcut');
assert(/createStartMenuShortcut:\s*true/.test(builder),
  'the end-user installer must create a Start Menu shortcut');
assert(/shortcutName:\s*Magic Pointer/.test(builder),
...
assert(exists('electron/update_manager.js'),
...
assert(/Add-Type\s+-AssemblyName\s+System\.Drawing/.test(smoke),
...
assert(/Get-Process\s+-Id\s+\$process\.Id/.test(smoke),
  'smoke cleanup must stay scoped to the launched PID');
assert(/\$children\.Count\s+-gt\s+0/.test(smoke),
  'process-tree traversal must not append a PowerShell null sentinel');
assert(/icon/i.test(smoke) && /version/i.test(smoke),
  'smoke script must inspect executable icon and version metadata');
assert(/package-smoke/i.test(smoke) && /Remove-Item/i.test(smoke),
