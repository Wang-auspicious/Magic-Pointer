...
assert(reader.includes("return '填入输入框（不发送）'"));
assert(reader.includes("proposal?.action_type === 'fabric_recipe_execute'"));
assert(reader.includes('plan.preview?.title'));
assert(reader.includes('plan.provider'));
assert(reader.includes('plan.risk'));
