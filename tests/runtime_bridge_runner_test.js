...
const { createRuntimeBridgeRunner } = require('../electron/runtime_bridge_runner');
...

{
  ...
  runner.run({ executable: process.execPath, args: [], input: {}, timeoutMs: 25, onComplete: value => delivered.push(value) });
  timeout();
  child.emit('close', 1);
...

{
  const child = fakeChild();
  const delivered = [];
  const runner = createRuntimeBridgeRunner({ spawnImpl: () => child });
  runner.run({
    executable: process.execPath, args: [], input: {}, timeoutMs: 1000,
...

{
  ...
  const delivered = [];
  const runner = createRuntimeBridgeRunner({ spawnImpl: () => child });
  runner.run({ executable: process.execPath, args: [], input: {}, timeoutMs: 1000, onComplete: value => delivered.push(value) });
...

{
  ...
  const crashedRunner = createRuntimeBridgeRunner({ spawnImpl: () => crashed });
  crashedRunner.run({
    executable: process.execPath, args: [], input: {}, timeoutMs: 1000,
...

{
  ...
  const delivered = [];
  const runner = createRuntimeBridgeRunner({ spawnImpl: () => child });
  runner.run({ executable: process.execPath, args: [], input: {}, timeoutMs: 1000, signal: controller.signal, onComplete: value => delivered.push(value) });
  controller.abort();
...

{
  ...
  runner.run({
    executable: process.execPath, args: [], input: {}, timeoutMs: 25,
    onProgress: () => {},
    onComplete: value => delivered.push(value),
  });
...
console.log('runtime bridge runner test ok');
